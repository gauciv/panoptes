...
const AnalyticsPage: React.FC<AnalyticsPageProps> = ({ subscriptions }) => {
  const {
    // totalWebhooks, // Optional: Can remove if you want to replace with Volume
    successRate,
    avgLatency,
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Latency analysis, value transfer, and traffic distribution
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {/* Card 1: Total Volume (Replaces generic event count) */}
        <StatCard
          title="Total Volume"
        ...
        <StatCard
          ...
          subtitle={topSourceWallet ? `${topSourceCount} events triggered` : 'No activity'}
      ...
      {/* TODO: Add Heatmap Chart Component here in next step */}
    </div>
  );
};
